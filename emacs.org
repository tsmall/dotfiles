...
#+BEGIN_SRC elisp
  (trs/set-default-font "Berkeley Mono" 150)
  (trs/set-variable-font "Verdana" 140)
#+END_SRC
...
I use the Zettelkasten method for my notes. So each file starts with a unique
ID (in YYYYMMDDHHMM format). And links to the files are just their IDs. By
default Org Mode doesn't know how to open those links. But Projectile does!

#+begin_src elisp
  (defun trs-org-open-zettel-link ()
    "Attempts to open an org link using a Zettelkasten ID."
  ...
  (add-to-list 'org-open-at-point-functions 'trs-org-open-zettel-link)
#+end_src
