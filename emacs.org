...
#+BEGIN_SRC elisp
  (trs/set-default-font  "Lexia Mono"     130)
  (trs/set-comment-font  "Lexia Mono"     130)
  (trs/set-variable-font "BBC Reith Sans" 140)
#+END_SRC
...
Recent versions of tab-bar-mode take key bindings that I use in other places,
for example to cycle visibility in taskpaper-mode. This undoes that.
...
#+begin_src elisp
  (use-package taskpaper-mode
    ...
    :bind (:map taskpaper-mode-map
           ("C-S-<up>" . taskpaper-outline-up-level)))
#+end_src
...
But make it easy to toggle them on for easier editing.

#+begin_src elisp
  (defun trs/toggle-org-markup ()
    "Toggle visibility of Org markup (bold, italics, etc) and links in current buffer."
    (interactive)
    (if org-hide-emphasis-markers
        (progn
          (setq-local org-hide-emphasis-markers nil)
          (setq-local org-link-descriptive nil)
          (message "Org markup and links shown"))
      (setq-local org-hide-emphasis-markers t)
      (setq-local org-link-descriptive t)
      (message "Org markup and links hidden"))
    (font-lock-flush)
    (font-lock-ensure))

  (define-key org-mode-map (kbd "C-c C-t") #'trs/toggle-org-markup)
#+end_src
...
I like being able to color some of the text in my notes. This code makes that possible with custom links.

#+begin_src elisp
  ;; (require 'org-colored-text)
#+end_src

But it doesn't (currently) play well with the way org mode folds text. This seems to fix it.

#+begin_src elisp
  ;; (setq org-fold-core-style 'text-properties)
#+end_src

I'm actually switching to a custom implementation that uses custom faces instead. That seems to be both much easier and much better behaved. Once I've migrated everything I'll probably delete the options above.

#+begin_src elisp
  (require 'org-custom-faces)
#+end_src
...
Verb is an extension to Org Mode that turns it into an API client.
...
Verb doesn't come with support for OAuth out of the box, but I often need it.
Luckily it does provide an API and hooks that made it possible for me to
implement it myself. My =verb-oauth1= package implements a Verb Request Mapping
Function to sign outgoing requests.
...
By default it fills to the window, which is too large.

#+begin_src elisp
  (setq nov-text-width 72)
#+end_src

I also like using a serif font for this, even though I use sans-serif for my
normal variable-pitch font.

#+begin_src elisp
  (setq nov-variable-pitch nil)
  (defun trs/nov-font-setup ()
    (buffer-face-set :family "BBC Reith Serif"
                     :height 180))
  (add-hook 'nov-mode-hook 'trs/nov-font-setup)
#+end_src
...
* Playing music
:PROPERTIES:
:header-args: :noweb-ref music-settings
:END:
