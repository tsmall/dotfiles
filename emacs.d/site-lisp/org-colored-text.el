...
;; I left the original copyright above, but I have modified this from its
;; original version. Here are the changes I made:

;; - Added a "bg" link type to set background color.
     ...
     (let ((rgb (assoc color color-name-rgb-alist))
	   ...
	   (progn
	     (setq r (* 255 (/ (nth 1 rgb) 65535.0))
		   g (* 255 (/ (nth 2 rgb) 65535.0))
		   b (* 255 (/ (nth 3 rgb) 65535.0)))
	     (format "<span style=\"background-color: rgb(%s,%s,%s)\">%s</span>"
		     (truncate r) (truncate g) (truncate b)
		     (or description color)))
	 (format "No Color RGB for %s" color)))))))
    ...
    (let* ((next-link (org-element-context))
	  ...
	  (progn
		...
		(- end post-blanks)
	     'bg t
	     'face
	     `((:background ,color :underline nil)))
	    (goto-char end))
	(goto-char limit)
	nil))))
